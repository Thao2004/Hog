...
def roll_dice(num_rolls, dice=six_sided):
    ...
    total_point = 0
    rolled_one = False # flag to check if any roll result is 1
    
    i = 1 # initialize the variable to keep track of num_rolls
...
def boar_brawl(player_score, opponent_score):
    ...
    boar_brawl_score = 3 * abs(opponent_tens_digit - player_one_digit)
    
    score = max(boar_brawl_score, 1)
    return score
...
def num_factors(n):
    ...
    count = 0 # variable to keep track of the number of factors of N
...
# Define another function called next_prime() to give us the next prime number greater than n
def next_prime(n):
    num = n + 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # Get the number of dice rolled for the first combination of score (0,0)
    num = strategy(0, 0)
    
    # Check all possible score combinations from 0 to goal - 1
    for player_score in range(goal):
        for opponent_score in range(goal):
            if strategy(player_score, opponent_score) != num:
                return False
    return True
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    max_average = 0 # Initialize the maximumm average score
    best_num_rolls = 1 # Initialize the best number of rolls
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    brawl_point = boar_brawl(score, opponent_score)
    
    if brawl_point >= threshold:
        return 0
